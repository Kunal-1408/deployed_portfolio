         ...
         <div className="grid grid-rows-1 md:grid-rows-3 gap-4 col-span-1">
          {whyChooseUs.map((whyChooseUs, index) => (
            <Card 
              ...
              onMouseEnter={() => {
                setActiveImage(whyChooseUs.imageUrl)
                setActiveIndex(index)
              }}
            >
              <CardContent className="p-6">
                <h2 className="text-xl font-semibold mb-2  transition-colors duration-300">{whyChooseUs.title}</h2>
                <p className="text-sm  transition-colors duration-300">{whyChooseUs.description}</p>
              </CardContent>
            </Card>
          ))}
        </div>
