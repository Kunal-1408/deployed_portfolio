...
export default function SocialProjects({ projects, filterTags = [] }: SocialProjectsProps) {
  ...
  const [hoveredProject, setHoveredProject] = useState<SocialProject | null>(null)
  const id = useId()
  ...
  const handleMouseEnter = (project: SocialProject) => {
  ...
  const handleClick = (project: SocialProject) => {
    setActive(project)
  }
