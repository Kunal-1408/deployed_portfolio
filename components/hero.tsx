...
export default function () {
  // Add a ref to track if the globe has been initialized
  ...
  // Pre-calculate colors to avoid random generation on re-renders
  const colors = ["#db8458", "#de7743", "#e08d63"]

  // Create a memoized version of the arc data to prevent regeneration
  const sampleArcs = useRef([
    {
      order: 1,
      startLat: -19.885592,
      startLng: -43.951191,
    ...
    // Keep the rest of your arcs, but use fixed color indices instead of random
    // For example:
    {
      order: 1,
      startLat: -19.885592,
    ...
    <section className="hero-section">
      <div className="flex flex-col md:flex-row items-center justify-center py-10 md:py-20 min-h-screen md:h-auto bg-gradient-to-b from-black from-0% via-neutral-600 w-full">
        <div className="max-w-7xl mx-auto w-full relative overflow-hidden h-full md:h-[42rem] px-4">
          ...
          <div className="mt-10 md:mt-0 md:absolute w-full md:-bottom-20 h-[300px] md:h-72 lg:h-full z-10 relative">
            {/* Only render the globe if it's been properly initialized */}
            {globeInitializedRef.current && <World data={sampleArcs} globeConfig={globeConfig} />}
          </div>
        </div>
      </div>
    </section>
